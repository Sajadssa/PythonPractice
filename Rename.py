...
class WordFileRenamer:
    ...
    def extract_info_from_pdf_filename(self, filename):
        """
        استخراج اطلاعات از نام فایل PDF
        
        فرمت فایل PDF: SJSC-GGNRSP-EPWC-REWK-[Sequence]-[Revision].pdf
        
        Args:
            filename: نام فایل PDF
            
        Returns:
            dict: دیکشنری حاوی sequence_number, revision, date
        """
        data = {
            'sequence_number': None,
            'revision': None,
        ...
        # بررسی فرمت استاندارد
        import re
        
        # الگوی استاندارد: SJSC-GGNRSP-EPWC-REWK-0064-G00.pdf
        pattern = r'SJSC-GGNRSP-[A-Z]+-REWK-(\d{4})-(G\d{2})'
        match = re.search(pattern, filename)
        
        if match:
            data['sequence_number'] = match.group(1)
            data['revision'] = match.group(2)
        
        return data
    
    def extract_date_from_pdf(self, file_path):
        """
        استخراج تاریخ از فایل PDF
        
        Args:
            file_path: مسیر فایل PDF
            
        Returns:
            str: تاریخ استخراج شده یا None
        """
        try:
            ...
            if len(reader.pages) > 0:
                ...
                # جستجوی الگوهای تاریخ (مثل: 11-Feb-2025)
                import re
                date_pattern = r'\d{1,2}-[A-Za-z]{3}-\d{4}'
                dates = re.findall(date_pattern, text)
                
                if dates:
                    return dates[0]  # اولین تاریخ پیدا شده
    ...
    def process_files(self, rename_files=False):
        ...
        Args:
            rename_files: آیا فایل‌های Word تغییر نام پیدا کنند؟
        ...
        # پردازش فایل‌های PDF (فقط خواندن اطلاعات از نام فایل)
        for file_path in pdf_files:
            print(f"📕 در حال پردازش PDF: {file_path.name}")
            
            try:
                # استخراج اطلاعات از نام فایل PDF
                data = self.extract_info_from_pdf_filename(file_path.name)
                
                # تلاش برای استخراج تاریخ از محتوای PDF
                pdf_date = self.extract_date_from_pdf(file_path)
                if pdf_date:
                    data['date'] = pdf_date
                ...
                result = {
                    'file_type': 'PDF',
                    'original_name': file_path.name,
                    'new_name': file_path.name,  # PDF ها قبلاً تغییر نام یافته‌اند
                    'sequence_number': data['sequence_number'] or 'N/A',
                    'revision': data['revision'] or 'N/A',
                    'date': data['date'] or 'N/A',
                    'status': 'موجود'
...
def main():
    ...
    source_dir = r"D:\Sepher_Pasargad\works\Production\Production Operation Report\Weekly"
