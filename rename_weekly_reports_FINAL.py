...
نسخه نهایی - دیباگ شده و آماده برای استفاده
تاریخ: دسامبر 2024
"""
...
def extract_from_word(doc_path):
    ...
    این تابع تمام جداول و پاراگراف‌ها را بررسی می‌کند و
    Sequence Number، Revision و Date را استخراج می‌کند.
    """
    print(f"   📄 پردازش Word...")
    ...
    try:
        ...
        # بررسی تمام جداول و تمام ردیف‌ها
        for table_idx, table in enumerate(doc.tables):
            # اگر همه اطلاعات پیدا شد، خارج شو
            if sequence_number and revision and date_obj:
                break
            
            # اگر جدول کمتر از 2 ردیف دارد، رد شو
            if len(table.rows) < 2:
                continue
            
            # بررسی تمام ردیف‌های جدول (به جز ردیف اول که معمولاً هدر است)
            for row_idx in range(1, len(table.rows)):
                row = table.rows[row_idx]
                
                # استخراج تمام مقادیر ردیف
                values = []
                ...
                # جستجو در مقادیر
                for val in values:
                    # Sequence Number: دقیقاً 4 رقم
                    if not sequence_number and val.isdigit() and len(val) == 4:
                        sequence_number = val
                        print(f"   ✅ Sequence: {val} (جدول {table_idx+1}, ردیف {row_idx+1})")
                    
                    # Revision: G + 2 رقم
                    if not revision and len(val) == 3 and val[0] == 'G' and val[1:].isdigit():
                        revision = val
                        print(f"   ✅ Revision: {val} (جدول {table_idx+1}, ردیف {row_idx+1})")
                    ...
                    if not date_obj:
                        ...
                        if temp_date:
                            date_obj = temp_date
                            date_str = val
                            print(f"   ✅ Date: {val} (جدول {table_idx+1}, ردیف {row_idx+1})")
        
        # اگر از جدول نتوانستیم بگیریم، از متن کامل استفاده می‌کنیم
        if not sequence_number or not revision:
            ...
            # الگوهای Regex برای پیدا کردن Sequence و Revision
            if not sequence_number or not revision:
                patterns = [
                    r'REWK\s+(\d{4})\s+(G\d{2})',      # REWK 0125 G01
                    r'REWK[|\s]+(\d{4})[|\s]+(G\d{2})', # REWK|0125|G01
                    r'(\d{4})\s+(G\d{2})',              # 0125 G01
                    r'(\d{4})[^\d]+(G\d{2})',           # 0125 ... G01
                ]
...
def main():
    ...
    # برای تست، از این خط استفاده کنید:
    # FOLDER = Path.cwd()
    
    if not FOLDER.exists():
        print(f"❌ پوشه پیدا نشد: {FOLDER}")
        print("\n💡 راهنمایی:")
        print("   1. مسیر را در خط 242 کد چک کنید")
